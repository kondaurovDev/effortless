...
/**
 * Simulates a pnpm monorepo:
 *
 *   workspace-root/
 *     node_modules/
 *       effect/          (hoisted)
 *       .pnpm/
 *         chromium@1.0.0/node_modules/chromium/
 *         linkedom@1.0.0/node_modules/linkedom/
 *     packages/
 *       core/            (workspace pkg, TS entry points → bundled, not in layer)
 *       infra/           (projectDir — where CLI runs)
 *         node_modules/
 *           core   -> ../../core
 *           effect -> ../../../node_modules/effect
 */
describe("extraNodeModules — monorepo", () => {
  let wsRoot: string;
  let infraDir: string;

  beforeAll(async () => {
    wsRoot = await fs.mkdtemp(path.join(os.tmpdir(), "layer-monorepo-"));

    // ── workspace root node_modules ──
    const rootNm = path.join(wsRoot, "node_modules");

    // effect — hoisted to root
    await fs.mkdir(path.join(rootNm, "effect"), { recursive: true });
    await fs.writeFile(
      path.join(rootNm, "effect", "package.json"),
      JSON.stringify({ name: "effect", version: "4.0.0" })
    );
    await fs.writeFile(
      path.join(rootNm, "effect", "index.js"),
      'module.exports = "effect";'
    );

    // chromium — only in .pnpm store (not hoisted)
    const chromiumStore = path.join(rootNm, ".pnpm", "chromium@1.0.0", "node_modules", "chromium");
    await fs.mkdir(chromiumStore, { recursive: true });
    await fs.writeFile(
      path.join(chromiumStore, "package.json"),
      JSON.stringify({ name: "chromium", version: "1.0.0" })
    );
    await fs.writeFile(
      path.join(chromiumStore, "index.js"),
      'module.exports = "chromium";'
    );

    // linkedom — only in .pnpm store (not hoisted)
    const linkedomStore = path.join(rootNm, ".pnpm", "linkedom@1.0.0", "node_modules", "linkedom");
    await fs.mkdir(linkedomStore, { recursive: true });
    await fs.writeFile(
      path.join(linkedomStore, "package.json"),
      JSON.stringify({ name: "linkedom", version: "1.0.0" })
    );
    await fs.writeFile(
      path.join(linkedomStore, "index.js"),
      'module.exports = "linkedom";'
    );

    // ── packages/core (workspace package with TS entry points) ──
    const coreDir = path.join(wsRoot, "packages", "core");
    await fs.mkdir(path.join(coreDir, "src"), { recursive: true });
    await fs.writeFile(path.join(coreDir, "src", "data.ts"), "export const x = 1;");
    await fs.writeFile(
    ...
    // ── packages/infra (projectDir) ──
    infraDir = path.join(wsRoot, "packages", "infra");
    const infraNm = path.join(infraDir, "node_modules");
    await fs.mkdir(infraNm, { recursive: true });
    await fs.writeFile(
    ...
    // Symlinks like pnpm creates
    await fs.symlink(coreDir, path.join(infraNm, "core"));
    await fs.symlink(path.join(rootNm, "effect"), path.join(infraNm, "effect"));
  });
  ...
  it("without extraNodeModules — chromium/linkedom not found, effect found", () => {
    const result = collectLayerPackages(infraDir, ["core", "effect"]);

    // core has TS entry points → filtered out
    expect(result.packages).not.toContain("core");
    expect(result.warnings.some(w => w.includes("core") && w.includes("TypeScript"))).toBe(true);

    // effect is symlinked in infra/node_modules → found
    expect(result.packages).toContain("effect");

    // chromium and linkedom are NOT in infra's node_modules → not found
    expect(result.packages).not.toContain("chromium");
    expect(result.packages).not.toContain("linkedom");
  });

  it("with extraNodeModules — chromium/linkedom found via workspace root", () => {
    const extraNodeModules = [path.join(wsRoot, "node_modules")];
    const result = collectLayerPackages(infraDir, ["core", "effect"], extraNodeModules);

    // core still filtered (TS entries)
    expect(result.packages).not.toContain("core");

    // effect still found
    expect(result.packages).toContain("effect");

    // chromium and linkedom now found via .pnpm in workspace root
    expect(result.packages).toContain("chromium");
    expect(result.packages).toContain("linkedom");
    expect(result.resolvedPaths.has("chromium")).toBe(true);
    expect(result.resolvedPaths.has("linkedom")).toBe(true);
  });

  it("ZIP includes workspace root packages", async () => {
    const extraNodeModules = [path.join(wsRoot, "node_modules")];
    const { packages, resolvedPaths } = collectLayerPackages(infraDir, ["core", "effect"], extraNodeModules);

    const zipResult = await Effect.runPromise(
      createLayerZip(infraDir, packages, resolvedPaths)
    );

    expect(zipResult.includedPackages).toContain("effect");
    expect(zipResult.includedPackages).toContain("chromium");
    expect(zipResult.includedPackages).toContain("linkedom");
    expect(zipResult.includedPackages).not.toContain("core");

    const zip = new AdmZip(zipResult.buffer);
    const entries = zip.getEntries().map((e: AdmZip.IZipEntry) => e.entryName);

    expect(entries).toContain("nodejs/node_modules/effect/package.json");
    expect(entries).toContain("nodejs/node_modules/chromium/package.json");
    expect(entries).toContain("nodejs/node_modules/linkedom/package.json");
    expect(entries.some((e: string) => e.includes("core/"))).toBe(false);
  });

  it("lockfile hash includes packages from extra dirs", async () => {
    const extraNodeModules = [path.join(wsRoot, "node_modules")];

    // Create a lockfile so computeLockfileHash doesn't fail
    await fs.writeFile(
      path.join(infraDir, "package-lock.json"),
      JSON.stringify({ name: "infra", lockfileVersion: 3, packages: {} })
    );

    const hash = await Effect.runPromise(computeLockfileHash(infraDir, extraNodeModules));
    expect(hash).toHaveLength(8);
    expect(hash).toMatch(/^[a-f0-9]+$/);

    // Hash without extra dirs should differ (fewer packages resolved)
    const hashWithout = await Effect.runPromise(computeLockfileHash(infraDir));
    expect(hash).not.toBe(hashWithout);

    await fs.rm(path.join(infraDir, "package-lock.json"));
  });
});
